"""Command line interface for docscriptor workflows."""
...
from docscriptor.compatibility import normalize_output_formats
from docscriptor.validation import DocumentValidationError
from docscriptor.workflows import build_python_document, convert_source, validate_source


def main(argv: Sequence[str] | None = None) -> int:
    """Run the docscriptor CLI."""
    ...
    try:
        ...
        print(f"docscriptor: {exc}", file=sys.stderr)
        return 2
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="docscriptor",
        description="Build, convert, and validate docscriptor documents.",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    build = subparsers.add_parser(
        "build",
        help="Build a Python-authored docscriptor document.",
    )
