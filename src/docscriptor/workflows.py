...
from docscriptor.compatibility import OutputFormat, normalize_output_formats
...
def _load_python_module(source_path: Path) -> ModuleType:
    module_name = f"_docscriptor_cli_{source_path.stem}"
...
def _coerce_document_candidate(value: object, *, name: str) -> Document | None:
    ...
    if candidate is None:
        return None
    raise TypeError(f"{name} must be a docscriptor.Document or return one")
